...
const StatCard: React.FC<StatCardProps> = ({ title, value, icon, type }) => (
  <>
    {type === 'mainCard' ? (
      <article
        className={
          'flex flex-col relative p-6 bg-[#9333EA] rounded-2xl shadow-md col-span-1 sm:col-span-2 lg:col-span-1 xl:col-span-2'
        }
      >
        ...
        <p className="text-white text-2xl xl:text-3xl font-extrabold mb-12">
          {value}
        </p>
      </article>
    ) : (
      <article className="flex flex-col relative p-6 bg-white rounded-2xl shadow-md lg:col-span-1 xl:col-span-1">
        <span className="flex items-center justify-between mb-3 w-full space-x-2">
